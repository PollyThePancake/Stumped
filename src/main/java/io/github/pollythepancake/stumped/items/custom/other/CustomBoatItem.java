...
public class CustomBoatItem {
    ...
    public CustomBoatItem(String id, boolean hasChest, Item planks, int burnTime) {
        BOAT_ITEM = TerraformBoatItemHelper.registerBoatItem(new Identifier(ModInit.MOD_ID, id + "_boat"), () -> boat, false, new FabricItemSettings().group(StumpedGroups.STUMPED_ITEMS).maxCount(1));
        CHEST_BOAT_ITEM = TerraformBoatItemHelper.registerBoatItem(new Identifier(ModInit.MOD_ID, id + "_chest_boat"), () -> boat, true, new FabricItemSettings().group(StumpedGroups.STUMPED_ITEMS).maxCount(1));
